...
![High-level system architecture.](./images/architecture-system-diagram.svg)
...
## Cross-chain communication: IBC and ICA
...
1. Validator calls                                                      
   createAttestation() on                                               
   ChallengeIssuer contract                                             
   -> returns attestationId                                             
   -> returns EIP-712 digest                                            
                                                                        
2. Validator submits signatures                                         
   -> gets 15-byte pointer                                              
   -> sets pointer in block                                             
      extraData: "SH" + pointer                                         
                                                                        
3. EVM Relayer scans extraData,     4. ShinzoHub processes              
   resolves pointer, verifies          MsgIndexerAssertion,             
   signatures, broadcasts              stores assertion                 
   MsgIndexerAssertion ----------->                                     
                                    5. Operator calls register()        
                                       on Indexer Registry (0x0212)     
                                       -> verifies assertion            
                                       -> derives DID/PID              
                                       -> ICA packet to SourceHub ---> 6. ACP adds DID to
                                                                           indexer group
```
...
1. Operator calls register()
   ...
   -> derives DID/PID
   -> ICA packet to SourceHub -----------> 2. ACP adds DID to
                                              host group
```
...
1. viewkit view deploy     2. View Registry (0x0210)
   -> encodes VWL bytes       -> decodes VWL header
   -> signs EVM tx             -> extracts SDL type name
   -> sends to 0x0210         -> deploys SVS-1 contract
                               -> ICA RegisterObject ----------> 3. ACP registers
                               -> emits Registered event            view object
                                          |
                                          v
...
Precompile Go code:
  ...
  |
  v
Hermes relayer picks up ICA packet (async)
  |
  v
