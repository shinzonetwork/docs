...
Repo: [shinzonetwork/shinzo-outpost](https://github.com/shinzonetwork/shinzo-outpost) (Solidity)
...
2. Outpost verifies the validator using the chain's native mechanism
3. Both the validator (withdrawal address) and the delegate sign a digest
4. A [relayer](relayer.md) picks up the signed assertion and broadcasts `MsgIndexerAssertion` to ShinzoHub
5. ShinzoHub verifies the assertion. The validator can now register in the Indexer Registry (`0x0212`).
...
```plaintext
Validator                        ChallengeIssuer Contract
    |                                      |
    | -- createAttestation(             -> |
    |      consensusPubKey,                |
    |      delegateKey)                    |
    |                                      |
    | <- attestationId + EIP-712 digest -- |
    |                                      |
    | -- submitAttestationSignature(    -> |
    |      attestationId,                  |
    |      withdrawalSig,                  |
    |      delegateSig)                    |
    |                                      |
    | <------- 15-byte pointer ----------- |
    |                                      |
    |  Validator sets pointer as block extraData:
    |  "SH" + 30 hex chars
    |  (only real block producers can write extraData)
```
...
3. A [relayer](relayer.md) picks up the event and delivers it to ShinzoHub as `MsgRequestStreamAccess`
