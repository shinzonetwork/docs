...
Repo: `shinzo-gh/shinzo-host-client` (Go)
...
```plaintext
  Indexer-1 ─┐
  Indexer-2 ─┼──> P2P (DefraDB replication)
  Indexer-3 ─┘             |
                            v
              ┌─────── Host Process ────────────────┐
              ...
              │          |                          │
              │          v                          │
              │  2. Create AttestationRecords       │
              │          |                          │
              │          v                          │
              │  3. Apply Lens transforms (WASM)    │
              │          |                          │
              │          v                          │
              │  4. Write View documents            │
              │          |                          │
              │          v                          │
              │  5. Serve GraphQL                   │
              └──────────┬──────────────────────────┘
           ...
           ┌─────────────┼─────────────┐
           v             v             v
     Port 444        Port 9181     Port 8080
  Playground (GraphQL) Internal API  Health/Metrics
```
...
- `"tm.event='Tx' AND EntityRegistered.key EXISTS"` -- new indexers/hosts joining
...
Code: `pkg/shinzohub/events.go`, lines 95-99.
...
```plaintext
Indexer A          Host                Indexer B         Other Hosts
    |               |                      |                  |
    | -- Block #1000 + BlockSignature --> |                   |
    |    (P2P)       |                      |                  |
    |                |  2. Verifies BlockSignature             |
    |                |     Creates AttestationRecord           |
    |                |     { attested_doc: "block_1000",       |
    |                |       vote_count: 1 }                   |
    |                |                      |                  |
    |                | <-- Block #1000 + BlockSignature ------ |
    |                |     (independently, P2P)                |
    |                |                      |                  |
    |                |  4. Verifies BlockSignature             |
    |                |     Upserts AttestationRecord:          |
    |                |     vote_count increments to 2          |
    |                |                      |                  |
    |                | ----------- P2P replication ----------> |
    |                |  5. P-counter merges automatically      |
    |                |     All hosts agree: vote_count = 2     |
```
...
2. `PostWasmToFile()` -- for each lens: base64-decodes the WASM bytes, validates the WASM magic number (`0x00 0x61 0x73 0x6D`), writes to disk with a sha256-derived filename
...
```plaintext
  Raw Log documents (from indexers)
                |
                v
          WASM Lens
          - Filter by contract address
          - ABI-decode event
          - Map to view schema fields
                |
                v
  View documents (USDCTransfer, etc.)
```
