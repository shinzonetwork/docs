...
```plaintext
Indexer-1 --+
Indexer-2 --+----- P2P (DefraDB replication) -----> HOST
Indexer-3 --+                                        |
                                                     |
                      +------------------------------+
                      |
...
```plaintext
1. Indexer A writes Block #1000 + BlockSignature
      | (P2P replication)
      v
2. Host receives Block #1000 + BlockSignature
   Verifies BlockSignature cryptographically
   Creates AttestationRecord { attested_doc: "block_1000", vote_count: 1 }
      |
3. Indexer B writes Block #1000 + BlockSignature (independently)
      | (P2P replication)
      v
4. Host receives same data from Indexer B
   Verifies BlockSignature
   Upserts AttestationRecord: vote_count increments to 2
      |
5. P2P replication between hosts
   P-counter merges automatically
...
1. `PostWasmToFile()` -- for each lens: base64-decodes the WASM bytes, validates the WASM magic number (`0x00 0x61 0x73 0x6D`), writes to disk with a sha256-derived filename.
