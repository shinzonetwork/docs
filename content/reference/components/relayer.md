...
Repo: [shinzonetwork/shinzo-evm-relayer](https://github.com/shinzonetwork/shinzo-evm-relayer) (Go)
...
Note: this is the EVM relayer, which bridges Ethereum to ShinzoHub. It is a completely different system from the Hermes IBC relayer, which bridges ShinzoHub to SourceHub. They share the word "relayer" and nothing else.
...
4. Broadcasts the message to ShinzoHub
...
### EVM assertion detection

The EVM relayer scans each new Ethereum block's `extraData` field for the "SH" tag. When found, it:

1. Extracts the 15-byte pointer from `extraData`
2. Resolves the pointer to an attestation on the ChallengeIssuer contract
3. Verifies the EIP-712 signatures
4. Sends `MsgIndexerAssertion` to ShinzoHub as a Cosmos SDK transaction
