...
The Indexer currently supports Ethereum Mainnet. It works with any execution client that exposes a standard Ethereum JSON-RPC and WebSocket interface (Geth, Reth, Nethermind, Erigon, or a managed provider). Supported chains can also be found at [shinzo.network/chains](https://shinzo.network/chains).

## How it fits together

For details on how the Indexer connects to the rest of the Shinzo stack, including DefraDB, the signing and attestation pipeline, and the P2P replication layer, see the [Architecture overview](/reference/architecture-overview/).
