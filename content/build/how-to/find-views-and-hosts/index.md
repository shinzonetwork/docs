...
description = "How to discover registered Views, serving Hosts, and pools on the Shinzo network using the Explorer, Studio, the REST API, or a script."
+++

Views, Hosts, and Generators are all registered on ShinzoHub, so discovery is really just reading the registry. There are four ways to do it: the Explorer in a browser, Shinzo Studio, the chain's REST API, or plain HTTP calls from your code. Pick whichever fits the job; they all read the same on-chain state.

Two fields come up constantly, so get to know them first:
...
[Shinzo Studio](https://studio.shinzo.network/) shows registered Views in its catalog, each with its definition, pool, and serving Hosts. Studio is the better browser experience when your question is "what Views exist and what do they return", because it puts the View's SDL next to its network status. You can also deploy your own View from the same UI; see [Create and deploy Views in Shinzo Studio](/build/how-to/use-shinzo-studio/).
...
A pool turns active once at least 3 Hosts have joined it, so `is_active` is your quick check for whether a View is being served. The registry moves over time, so expect different names and addresses when you run these.

## Query the REST API from a script

The same registry reads are available from any HTTP client when your app needs them programmatically; the REST API is the integration point. A TypeScript example using `fetch`:

```ts
const REST = "http://testnet.shinzo.network:1317";

const { views } = await fetch(
  `${REST}/shinzonetwork/view/v1/views?pagination.limit=25&include_metadata=true`,
).then((r) => r.json());

const { hosts } = await fetch(
  `${REST}/shinzonetwork/host/v1/hosts?pagination.limit=100`,
).then((r) => r.json());

const { details: pools } = await fetch(
  `${REST}/shinzonetwork/pool/v1/views/${views[0].address}/pools`,
).then((r) => r.json());
```

`/shinzonetwork/view/v1/views` returns registered Views (with the parsed query, SDL, and lens metadata when `include_metadata` is set), `/shinzonetwork/host/v1/hosts` returns Host clients with their `endpoint_address`, and `/shinzonetwork/pool/v1/views/{address}/pools` returns the pools for a View with member Hosts and an `is_active` flag. The [Query your first View](/build/tutorials/query-your-first-view/) tutorial walks through tying these together into a full signed query flow.
...
This is the same info a Host publishes to the registry when it registers, so it's the fastest way to check what a specific Host claims to be. See [Connect your app to a Host](/build/how-to/connect-to-a-host/) for the health endpoint and the rest of the port layout.

## What does not exist

Viewkit has no list or discovery commands. It builds and deploys View bundles, and that's all. Discovery happens on-chain through the paths above, or through the Explorer and Studio UIs listed in [Tools](/reference/tools/).
