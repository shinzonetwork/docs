...
1. Get testnet SHNZ from the [faucet](https://faucet.shinzo.network/) if you don't have any. Deploying a View and funding a query balance are on-chain transactions, so the wallet needs a small balance for network fees on top of whatever you spend.
...
1. Review the generated View definition: the source query, the SDL, and the lens definition with its arguments. This is exactly the bundle Viewkit would produce for the same inputs, and it's worth reading before you sign anything.
...
After registration, Studio prompts you to back the View with demand. Creating demand bonds SHNZ to the View's pool and is how Host clients see that the View should be served. The first demand creates the pool, and the pool turns active once at least 3 Hosts have joined it. Until then Studio shows the pool as waiting for Hosts.
...
1. Write or adjust the GraphQL query in the editor and run it. Each run asks your wallet for one signature and spends from your prepaid query balance, so keep an eye on the balance Studio shows. If it runs out, Studio won't send and tells you to add funds first.
