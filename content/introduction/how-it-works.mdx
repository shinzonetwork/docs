...
At its simplest, Shinzo has four kinds of moving parts: 
...
## The data's journey: from block to app

To make this concrete, let's follow a single ERC-20 token transfer — say, a USDC transfer on Ethereum Mainnet — from the moment it lands on-chain to the moment an app shows it to a user.

![](./images/data-journey-black.svg)

### 1. A block arrives at a validator
...
### 2. The Indexer structures and signs it
...
### 4. A View transforms primitives into something useful
...
This walkthrough introduces each player in the order they touch the data:
...
**Developers** don't run anything on the network. They write Views with `viewkit`, deploy them to ShinzoHub, and build applications that subscribe to the resulting data. See [Build a View](#) to start.
...
A few design choices shape the rest of the stack, and it's worth pausing on them.
