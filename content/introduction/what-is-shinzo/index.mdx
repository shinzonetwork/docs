...
This setup is expensive, introduces a significant trust-gap, and there's a lot of places where things can go wrong. The DNS of the indexer might fail, or the cloud service where they host their applications might go down. On top of that, there's no way to verify that the data you're receiving is true until _after_ you've received (and paid) for it.
...
:::tip
It's important to note here that you _do not_ need to run a Validator node in order to run an Indexer. You can simply connect and Indexer to a third-party Validator node. Validator node and the Shinzo Indexer _do not_ need to be on the same network.
:::
