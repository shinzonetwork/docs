...
You don't need a wallet, funds, or a ShinzoHub registration for this quickstart. Registration is what lets your operators participate in the network and earn rewards. It's covered on the [Indexer registration](/indexers/register) and [Host registration](/hosts/register) pages.
...
`BOOTSTRAP_PEER` is a libp2p multiaddr that essentially saus _"speak IPv4 to this address on this TCP port, then handshake with this Peer ID."_ If the Peer ID doesn't match, the Host refuses the connection. This is what makes the connection authenticated end to end.
...
The rows that come back started life as logs on Ethereum, were pulled in by the Indexer over the Geth WebSocket, signed, gossiped over libp2p to the Host, and are now being served back to you over GraphQL. More queries are on the [Host examples](/hosts/examples) page.
