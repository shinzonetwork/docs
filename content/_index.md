...
**Generators** sit next to blockchain nodes and turn new blocks into structured documents as they arrive. They cryptographically sign everything they produce, so anyone downstream can check that the data came from a real generator (and ideally several of them).

**Hosts** receive that primitive data over a peer-to-peer network and apply user-defined transforms called _Views_. They also keep attestation records, which count how many different generator signed off on each piece of data. Applications can use those counts to set their own trust thresholds.
...
Hosts are configured through a YAML file that lists which Views to subscribe to, the minimum number of Generators that must have signed a document before it's accepted, and where to listen for subscriber connections. New Views can be picked up and served without restarting.
...
- The Generator ingests Ethereum Mainnet from a Geth node, signs documents, and replicates them over DefraDB's libp2p layer.
